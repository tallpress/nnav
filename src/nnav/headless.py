...
def filter_messages(
    ...
    # Type filter
    if filter_type:
        type_map = {"REQ": MessageType.REQUEST, "RES": MessageType.RESPONSE, "PUB": MessageType.PUBLISH}
    ...
    if subject_pattern:
        ...
        pattern = subject_pattern.replace(".", r"\.").replace("*", r"[^.]+").replace(">", r".+")
        regex = re.compile(f"^{pattern}$")
        result = [m for m in result if regex.match(m.subject)]
    ...
    if filter_text:
        if filter_text.startswith("/") and filter_text.endswith("/") and len(filter_text) > 2:
            ...
            try:
                ...
                result = [m for m in result if regex.search(m.subject) or regex.search(m.payload)]
            except re.error:
                print(f"Warning: Invalid regex pattern: {filter_text}")
        else:
            # Text filter
            filter_lower = filter_text.lower()
            result = [m for m in result if filter_lower in m.subject.lower() or filter_lower in m.payload.lower()]

    return result


def export_messages(messages: list[NatsMessage], path: Path, format: str) -> None:
